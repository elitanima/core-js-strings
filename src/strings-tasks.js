...
function isPalindrome(str) {
  return (
    ...
    str.toLowerCase().match(/\w/gi).join('')
  );
}
...
function reverseWords(str) {
  ...
  const reverseStr = [];
  for (let i = 0; i < arr.length; i += 1) {
    const elem = arr[i].split('').reverse().join('');
    reverseStr.push(elem);
  }
...
function encodeToRot13(str) {
  ...
  const abc2 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(/[a-z]/gi, (elem) => abc2[abc.indexOf(elem)]);
}
